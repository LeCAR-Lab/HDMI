"""Replay OmniRetarget trajectories inside the HDMI (mjlab) environment."""
...
ROBOT_ROOT_DOFS = 7
OBJECT_ROOT_DOFS = 7
ROBOT_JOINT_DOFS = len(ROBOT_JOINT_NAMES)


def convert_qpos_to_env_order(qpos: np.ndarray) -> np.ndarray:
    """Reorder OmniRetarget qpos to match MuJoCo (translation first) ordering."""
    if qpos.ndim != 2:
        raise ValueError(f"Expected qpos with shape (T, D), got {qpos.shape}")

    num_dofs = qpos.shape[1]
    has_object = num_dofs == ROBOT_ROOT_DOFS + ROBOT_JOINT_DOFS + OBJECT_ROOT_DOFS
    if num_dofs not in (
        ROBOT_ROOT_DOFS + ROBOT_JOINT_DOFS,
        ROBOT_ROOT_DOFS + ROBOT_JOINT_DOFS + OBJECT_ROOT_DOFS,
    ):
        raise ValueError(
            f"Unsupported qpos dimension {num_dofs}. "
            "Expected 36 (robot) or 43 (robot+object)."
        )

    base_quat = qpos[:, 0:4]
    base_pos = qpos[:, 4:7]
    robot_joint_pos = qpos[:, 7 : 7 + ROBOT_JOINT_DOFS]

    reordered = [base_pos, base_quat, robot_joint_pos]

    if has_object:
        obj_quat = qpos[:, 7 + ROBOT_JOINT_DOFS : 7 + ROBOT_JOINT_DOFS + 4]
        obj_pos = qpos[:, 7 + ROBOT_JOINT_DOFS + 4 : 7 + ROBOT_JOINT_DOFS + 7]
        reordered.extend([obj_pos, obj_quat])

    return np.concatenate(reordered, axis=-1).astype(np.float32)


def load_trajectory(npz_path: Path) -> tuple[np.ndarray, float]:
    """Load qpos trajectory (and fps) from an OmniRetarget npz file."""
    data = np.load(str(npz_path))
    if "qpos" not in data:
        raise KeyError(f"'qpos' entry is missing in {npz_path}")
    qpos = convert_qpos_to_env_order(data["qpos"])
    fps = float(data["fps"]) if "fps" in data else 30.0
    return qpos, fps


def setup_env(task: str = "move_largebox", max_episode_length: int = 1000):
...
def main(
    trajectory_path: str = "../../OmniRetarget_Dataset/robot-object/sub3_largebox_000_original.npz",
    task: str = "move_largebox",
):
    ...
    qpos, fps = load_trajectory(trajectory_path)
    print(f"Loaded qpos sequence with shape: {qpos.shape} at {fps:.2f} FPS")

    num_steps = qpos.shape[0]
    print(f"Setting up HDMI environment for task: {task}")
    env = setup_env(task=task, max_episode_length=num_steps)
    device = env.device

    if qpos.shape[1] != env.sim.mj_model.nq:
        raise ValueError(
            f"qpos dimension mismatch: traj has {qpos.shape[1]} while env expects {env.sim.mj_model.nq}"
        )

    ctrl_torch = torch.zeros(num_steps, env.sim.mj_model.nu, dtype=torch.float32, device=device)
    qpos_torch = torch.from_numpy(qpos).to(device)
    qvel_torch = torch.zeros(num_steps, env.sim.mj_model.nv, dtype=torch.float32, device=device)
    ...
    print(f"Rolling out {ctrl_torch.shape[0]} control steps...")
    ...
    motion_frames = []
    for i in range(ctrl_torch.shape[0]):
