...
class eef_contact_exp(RobotObjectTrackReward):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.gain = gain
        self.eef_pos_error = torch.zeros(
            self.num_envs, self.command_manager.num_eefs, device=self.device
        )
        self.eef_frc = torch.zeros(
            self.num_envs, self.command_manager.num_eefs, 3, device=self.device
        )
    ...
    def update(self):
        ...
        self.eef_pos_error[:] = (
            eef_pos_diff.norm(dim=-1) - self.pos_tolerance
        ).clamp_min(0.0)
    ...
    def compute(self):
        if isinstance(self.frc_thres, float):
            ...
            contact_frc = (
                (self.eef_frc.abs() - self.frc_thres).clamp_max(0.0).mean(dim=-1)
            )

        rew = torch.exp(-self.eef_pos_error / self.pos_sigma) * torch.exp(
            contact_frc / self.frc_sigma
        )
        # shape: [num_envs]
        rew = (
            rew * self.in_range.float() * self.gain + 1 - self.in_range.float()
        ).mean(dim=-1)
...
class eef_contact_exp_max(RobotObjectTrackReward):
    def __init__(
        ...
        **kwargs,
        ...
        self.eef_ori_error = torch.zeros(
            self.num_envs, self.command_manager.num_eefs, 3, device=self.device
        )
        self.eef_frc = torch.zeros(
            self.num_envs, self.command_manager.num_eefs, 3, device=self.device
    ...
    def compute(self):
        ...
        # shape: [num_envs]
        return (rew.max(dim=-1).values * self.in_range.any(dim=-1).float()).unsqueeze(
            -1
        )
